<script>
  ...
  $('#date,#cheque_issue_date').daterangepicker({
    ...
    function formRequest(){
      ...
      axios.post('{{route("sales_return.store")}}',formData)
      .then(response=>{
  ...
  function searchInvoice(element){
    //  console.log(element);
     invoice_id=element.value.substr(6,20)
    //  console.log(invoice_id)
     if(event.key=='Enter'){
      console.log(invoice_id)
      axios.get("{{URL::to('admin/invoice-data')}}/"+invoice_id)
      .then(res=>{
        console.log(res)
        showInvoice(res.data[0])
      })
     }
  }
