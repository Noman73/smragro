...
window.formRequest= function(){
    ...
    //axios post request
    if (id==''){
         axios.post("{{route('credit-setup.store')}}",formData)
      ...
      axios.post("{{URL::to('admin/credit-setup/')}}/"+id,formData)
        .then(function (response){
          if(response.data.message){
              toastr.success(response.data.message);
              datatable.ajax.reload();
...
$(document).delegate(".editRow", "click", function(){
    ...
    .then((data)=>{
      ...
      editKeys.forEach(function(key){
         ...
         if(key=='customer'){
          $('#'+'customer').html('<option value="'+data.data['customer']['id']+'">'+data.data['customer']['name']+'</option>');
        }
        if(key=='amount'){
          $('#amount').val(Math.abs(data.data[key]));
        }
         $('#modal').modal('show');
         $('#id').val(data.data.id);
      })
...
function addItem(){
  let html="<tr>";
      html+="<td><select class='form-control customer' name='customer[]'></select></td>";
...
//   }
 ...
 function getSetPrice(){
    ...
    .then((res)=>{
      ...
      let html="<tr>";
      res.data.forEach(function(d){
        html+="<td><select class='form-control customer' name='customer[]'><option value='"+d.product_id+"'>"+d.product.name+"</option></select></td>";
