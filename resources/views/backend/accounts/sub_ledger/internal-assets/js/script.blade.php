...
window.formRequest= function(){
    ...
    let name=$('#name').val();
    let account_ledger=$('#account_ledger').val();
    let id=$('#id').val();
    let formData= new FormData();
    formData.append('name',name);
    formData.append('account_ledger',account_ledger);
    ...
    //axios post request
    if (id==''){
         axios.post("{{route('sub_ledger.store')}}",formData)
      ...
      axios.post("{{URL::to('admin/sub_ledger/')}}/"+id,formData)
        .then(function (response){
          if(response.data.message){
              toastr.success(response.data.message);
              datatable.ajax.reload();
...
$("#account_ledger").select2({
    ...
    allowClear:true,
    ajax:{
      url:"{{URL::to('/admin/accounts/get-account-ledger-sub')}}",
      type:'post',
      dataType:'json',
      delay:20,
      data:function(params){
