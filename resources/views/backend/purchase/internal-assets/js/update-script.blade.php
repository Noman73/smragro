<script>
    ...
    $('#date,#cheque_issue_date').daterangepicker({
      ...
      function formRequest(){
        ...
        // }
        axios.post("{{route('purchase.update',$invoice[0]['id'])}}",formData)
        .then(response=>{
    ...
    function showInvoice(invoices){
        if(invoices.supplier_id==null){
            $('#cash').attr('checked',true);
        }else{
            $('#regular').attr('checked',true);
        }
        if(invoices.supplier!=null){
            $('#supplier').html('<option value="'+invoices.supplier.id+'">'+invoices.supplier.name+'('+invoices.supplier.phone+')'+'</option>')
        }
        ...
        // res=[];
        html="";
        for(i=0;i<invoices.purchase.length;i++){
            console.log(invoices.purchase[i]);
            d=invoices.purchase[i];
