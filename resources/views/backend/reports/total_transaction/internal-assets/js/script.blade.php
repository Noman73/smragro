...
//   }
  ...
  function Apply(){
    ...
    axios.post("{{URL::to('/admin/total-transaction-report')}}",{from_date:fromDate,to_date:toDate})
    .then((res)=>{
      console.log(res)
      html="";
      total=0;
      ...
      if(type==d.type || type==''){
        ...
        if(d.type=="Receive"){
            link="<a href='"+"{{URL::to('/admin/view-pages/receive-view/')}}/"+d.id+"'>"+'R-'+dateFormatTrxID(d.date*1000)+(d.id)+"</a>"
        }else if(d.type=='Payment'){
            link="<a href='"+"{{URL::to('/admin/view-pages/payment-view/')}}/"+d.id+"'>"+'P-'+dateFormatTrxID(d.date*1000)+(d.id)+"</a>"
        }else{
            link="<a href=''>"+'J-'+dateFormatTrxID(d.date*1000)+(d.id)+"</a>"
        }
          trx_id='<>'
          html+="<tr>"
          html+="<td class='text-left'>"+dateFormat(d.date*1000)+"</td>"
          html+="<td class='text-center'>"+link+"</td>"
          html+="<td class='text-left'>"+d.type+"</td>"
      ...
      // html+="<th colspan='4'>Total</th><th>"+total+"</th>";
      $('#data-load').html(html);
  ...
  function dateFormatTrxID(data){
    date=new Date(data);

    let dates = ("0" + date.getDate()).slice(-2);
    let month = ("0" + (date.getMonth() + 1)).slice(-2);
    let year = (date.getYear().toString()).substr(1);;
    return(dates + month +  year);
    // return date;
  }
