...
//   }
  ...
  function Apply(){
    ...
    let subledger=$('#supplier').val();
    $('#inv_from_date').text(fromDate);
    $('#inv_to_date').text(toDate);
    if(subledger!=null){
      
      $('#inv_ledger').text($('#supplier option:selected').text());
    }else{
      $('#inv_ledger').text($('#supplier option:selected').text());
    }
    $('.inv_ledger').removeClass('d-none')
    axios.post("{{URL::to('/admin/supplier-statement-report')}}",{ledger:ledger,subledger:subledger,from_date:fromDate,to_date:toDate})
    .then((res)=>{
      console.log(res)
      html="";
      ...
      res.data[0].forEach(function(d){
        console.log(d);
          ...
          if(class_type==1 || class_type==4){
            ...
            html+="<td class='text-right'>"+(balance+=parseFloat(d.credit)-parseFloat(d.debit))+"</td></tr>";
          } 
      })
      $('#data-load').html(html);
    })
