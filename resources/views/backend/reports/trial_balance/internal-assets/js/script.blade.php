...
//   }
  ...
  function Apply(){
    ...
    axios.post("{{URL::to('/admin/trial-balance')}}",{from_date:fromDate,to_date:toDate})
    .then((res)=>{
      console.log(res)
      html="";
      dr_count=0;
      ...
      res.data.forEach(function(d){
        console.log(d);
          ...
          html+="<tr><td class='text-left'>"+d.class_name+"</td>";
          html+="<td class='text-left'>"+d.code+(d.code==''? "" : '-')+d.name+"</td>";

          if(d.class_type==1 || d.class_type==4){
            dr_count+=parseFloat(d.debit-d.credit)
            html+="<td class='text-right' style='border:1px solid black;'>"+(parseFloat(d.debit-d.credit)).toFixed(2)+"</td>";
            html+="<td class='text-right' style='border:1px solid black;'>"+'0.00'+"</td></tr>";
          }else{
            cr_count+=parseFloat(d.credit-d.debit);
            html+="<td class='text-right' style='border:1px solid black;'>"+'0.00'+"</td>";
            html+="<td class='text-right' style='border:1px solid black;'>"+(parseFloat(d.credit-d.debit)).toFixed(2)+"</td></tr>";
          }
          
      })
      $('#data-load').html(html);
      $('#dr_count').text(dr_count.toFixed(2));
      $('#cr_count').text(cr_count.toFixed(2));
