...
class FundTransferController extends Controller
{
    ...
    {
        $journalInvoice=VInvoice::where('id',$id)->first();
        if($journalInvoice==null){
            return redirect(URL::to('admin/accounts/fund-transfer'));
        }
        $invoice=DB::select("
        ...
        ",['id'=>$id]);
        // dd($invoice);
        return view('backend.view_pages.fund_transfer.fund_transfer',compact('invoice','journalInvoice'));
    }
    public function print($id){
        ...
        $invoice=DB::select("
            ...
            order by voucers.id
        ",['id'=>$id]);
        return view('backend.view_pages.fund_transfer.print',compact('invoice','journalInvoice'));
    }
}
