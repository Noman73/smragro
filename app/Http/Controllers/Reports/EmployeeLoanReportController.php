...
class EmployeeLoanReportController extends Controller
{
    ...
    {
        return view('backend.reports.employee_loan.employee_loan');
    }
    public function getReport()
    {
        $data=DB::select("
            SELECT id,code,name,phone,adress,(ifnull((select sum(voucers.debit-voucers.credit) from voucers
            left join account_ledgers on voucers.ledger_id=account_ledgers.id and account_ledgers.name='Employee Loan'
            WHERE account_ledgers.name='Employee Loan' and voucers.subledger_id=employees.id
