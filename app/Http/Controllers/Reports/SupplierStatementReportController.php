...
class SupplierStatementReportController extends Controller
{
    ...
    {
        return view('backend.reports.supplier_statement.supplier_statement');
    }
    public function generateReport(Request $request)
    {
        ...
        if($validator->passes()){
           ...
           $supplier_ledger=AccountLedger::where('name','Supplier')->first()->id;
            ...
            ",['ledger_id'=>$supplier_ledger,'subledger'=>$request->subledger,'from_date'=>$from_date]);
            $arr= DB::select("
            ...
            order by voucer.date,voucer.id
                ",['ledger_id'=>$supplier_ledger,'subledger'=>$request->subledger,'from_date'=>$from_date,'to_date'=>$to_date]);
